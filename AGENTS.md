# AGENTS.md — Hausregeln für Coding-Projekte (Speakwerk)

Vorlage erstellt am 17.05.2026 — angepasst für Speakwerk am 09.06.2026.

Diese Datei enthält die Hausregeln für den implementierenden Agenten (Gemini).
Sie ist eng mit `CLAUDE.md` synchronisiert, unterscheidet sich jedoch in der Agenten-Rolle (Implementierung vs. Review) und in der Commit-Co-Author-Zeile.
...
## Rollen und Workflow (Gemini)

Gemini arbeitet als **implementierender Entwickler**:
- Codeänderungen, Formatierungen, Dateierstellung und lokale Commits werden eigenständig im Rahmen des freigegebenen Implementierungsplans durchgeführt.
- Features werden immer auf einem separaten Feature-Branch (`feature/xyz`) entwickelt.
- Sobald ein Feature-Schritt abgeschlossen ist, wird ein Pull Request vorbereitet oder Änderungen zur Review an Claude übergeben.
